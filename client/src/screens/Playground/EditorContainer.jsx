    ...
    <div className="editor-container">
      <div className="upper-tool-bar">
        <div className="header">
          ...
          <button className="button" onClick={saveCode}>
            Save code
          </button>
        </div>
      ...
      <div className="lower-tool-bar">
        <label htmlFor="codefile">
          <input type="file" accept="." id="codefile" onChange={(e) => getFile(e, setCurrentCode)} /> <BiImport />{" "}
          Import Code
        </label>
        <a href={`data:text/plain;charset=utf-8,${encodeURIComponent(currentCode)}`} download="code.txt">
          <BiExport /> Export Code
        </a>
        <button className="save-and-run-button" onClick={runCode}>
          Run Code
        </button>
      </div>
    </div>
